...
export async function POST(request: NextRequest) {
    const { userId, userWishlistItems }: SyncSemanticStoreRequest = await request.json();

    console.log(`\n--- Sync request received ---`);
    console.log(`User ID: ${userId}`);
    console.log(`Incoming items count: ${userWishlistItems.length}`);
    console.log(`Incoming items:`, userWishlistItems.map(i => i.id));
    ...
    try {
        ...
        if (userWishlistItems.length > documentCount) {
            console.log(`Adding ${userWishlistItems.length - documentCount} missing items...`);

            const existingItemsResponse = await semanticSearchClient.index(userId).range({
                cursor: "0",
                limit: documentCount,
            });

            console.log(`Fetched ${existingItemsResponse.documents.length} existing items from index`);
            const existingIds = new Set(existingItemsResponse.documents.map(item => item.id));
            const itemsToAdd = userWishlistItems.filter(item => !existingIds.has(item.id));

            console.log(`Items to add:`, itemsToAdd.map(i => i.id));
        ...
        // If database has fewer items, remove the extra ones
            ...
            console.log(`Removing ${documentCount - userWishlistItems.length} extra items...`);
        ...
        // If counts match, just update all items
            ...
            console.log(`Counts match, updating all ${userWishlistItems.length} items...`);
        ...
        return new Response(JSON.stringify({ error: "Failed to sync wishlist items" }), { status: 500 });
    }
}
